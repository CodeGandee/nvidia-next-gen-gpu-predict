...
本文研究的不是“按历代增速猜下一代”，而是一个带时间因果约束的问题：在每一历史世代的规格代理决策时点之前，公开观察者能够看到哪些工作负载、制程、HBM、封装、互连、功耗与软件信号；这些信号如何移动可行域并改变联合规格选择；同一方法在滚动历史留出中是否优于朴素基线。证据库含 34 项材料，其中 16 篇学术论文均完成全文研读，32 项来源就绪，TSMC 2026 技术材料与 JEDEC HBM4 材料因访问粒度受限保留为部分证据。我们在一个答案隔离的 9 事件历史面板上比较三条不可混合的路线，并公开报告输入、拟合、逐折误差、拒绝理由和约束敏感性。路线 A 预声明 6 个指标×3 种解释变量组合：8 个变体在拟合前因无可评分折而拒绝，10 个完成拟合；后者形成的 30 个“变体×前置期”资格单元全部失败，故不发布统计中心。路线 B 的六折回测揭示 12 个目标根变量中 7 个仍依赖显式假设，且低精度代理的平均绝对对数误差为 0.982；它只用于敏感性和影响方向，不代表 NVIDIA 内部决策网络。按预声明发布规则，首页数值供给资格为 $(A,B,C)=(0,0,1)$；这是一项门控选择，不是模型平均。路线 C 在 3 个时点、5 个架构条件与 5 组目标权重上形成 75 个条件，每条件抽样 1024 次，并显式枚举封装、良率、HBM、SerDes、功耗、热与机架约束。其 24 个月、balanced、架构未决条件给出的边际工程工作点为：471.8B 晶体管、296 SM、768 GB HBM、45.3 TB/s HBM 带宽、43.4 PFLOP/s 稠密 FP4、9.18 TB/s 双向 NVLink、2.6 kW 单 GPU、72 GPU 与 323 kW 机架；这些数值不是最可能值，同条件 q10/q90 不是置信区间，跨 25 条件外包络也没有覆盖概率。GTC 2026 对 Feynman 的官方边界仅包括 2028 路线图位置、Die Stacking、Custom HBM、Rosa CPU、BlueField-5、NVLink 8 CPO、Spectrum-7 204T CPO 与 ConnectX-10，并未披露任何 GPU 数值规格。全文据此把可支持结论、条件工程判断与未知严格分开，并给出逐项反证和更新规则。

## 结论先行：Feynman 规格预测

**结论先行：Feynman 下一代数据中心 GPU 的条件参数预测。** 主预测置顶，两个较低权重的工程情景、低置信影响图诊断和被回测拒绝的统计路线依次列后。这里的“综合”是按预声明发布资格逐路门控，不是把三组数字加权平均：A 检验统计外推资格，B 审计公开代理与显式假设的传播，C 生成具名条件下的联合可行工作点。

| 排序与模型 | 晶体管 / SM | HBM 容量 / 带宽 | 计算 / NVLink | 单 GPU TDP | 机架 GPU / 功率 | A/B/C 综合与含义 |
|---|---:|---:|---:|---:|---:|---|
| 主预测（首选）；C：24m、balanced、架构未决 | 471.8B / 296；413–508B / 248–336 | 768 GB / 45.3 TB/s；576–1024 / 33.5–50.7 | FP4 43.4 PF / 9.18 TB/s；35.7–52.6 / 8.28–10.07 | 2.6 kW；2.6–3.0 | 72 / 323 kW；72–144 / 290–511 | 首页数值资格 $(A,B,C)=(0,0,1)$；A 为 0 因 0/30 合格，B 为 0 因只预声明为诊断且 7/12 根为假设；故本行只由 C 生成。 |
| 次级情景 1；C：2T–1L | 311.2B / 192 | 576 GB / 33.2 TB/s | FP4 34.7 PF / 9.18 TB/s | 2.2 kW | 144 / 448 kW | $(A,B,C)=(0,0,1)$；仅为 C 的固定 2T–1L 较低权重情景；2T 模型内边际频率 16.8%。 |
| 次级情景 2；C：4T–2L | 908.5B / 584 | 768 GB / 44.0 TB/s | FP4 44.2 PF / 9.17 TB/s | 2.6 kW | 72 / 320 kW | $(A,B,C)=(0,0,1)$；仅为 C 的固定 4T–2L 压力上界；2L 模型内边际频率 8.3%。 |
| 低置信诊断；B：影响图 | 672B / — | 576 GB / 56 TB/s | 低精度代理 200 PF / 7.2 TB/s | 2.8 kW | — / — | B 的首页数值资格为 0；该行只诊断假设传播，C 不采用此中心，也不与主行平均。 |
| 回测拒绝；A：环境回归 | — / — | — / — | — / — | — | — / — | A 的首页数值资格为 0；0/30 资格单元合格意味着模型弃权，B/C 不使用其外推数值。 |

读表规则：表中 0/1 是“能否向首页供应数值工作点”的资格指示量，不是概率、置信度或模型平均权重。A 的否决使全文不能声称存在经历史回测资格审定的统计中心；B 只提供方向与敏感性诊断；C 的 1 只允许发布具名工程工作点。24m 指 2026-03-20 信息截点。路线 C 分位数没有历史覆盖概率；16.8% 与 8.3% 是模型内边际选择频率，不是 NVIDIA 采用概率。路线 B 的低精度代理不等同于稠密 FP4。官方 Feynman GPU 数值规格截至证据冻结日仍为 0 {cite}`nvidia2026feynman`。
...
### 门控式综合规则：表 1 如何由 A、B、C 得出

表 1 的“综合”是结果资格选择，不是数值集成。令 $\gamma_r^{home}\in\{0,1\}$ 表示路线 $r$ 是否按预声明发布规则有资格向首页供应数值工作点。路线 A 只有至少一个“变体×前置期”单元同时通过全部资格门槛时才可能取 1；实际 0/30 合格，所以 $\gamma_A^{home}=0$。路线 B 从试验设计开始就只承担假设传播和敏感性诊断，没有 decision/utility 节点，也没有发布中心的规则，所以 $\gamma_B^{home}=0$；7/12 目标根为显式假设及其回测误差进一步说明不能事后升级。路线 C 的发布对象不是统计中心，而是具名时点、架构与目标权重下的工程工作点；75 个条件均按同一可行域程序执行，因此 $\gamma_C^{home}=1$。

在本次已观测结果下，选择关系为

$$
(\gamma_A^{home},\gamma_B^{home},\gamma_C^{home})=(0,0,1),\qquad
r^*=\operatorname{Select}_{\mathcal P}\{A,B,C\}=C,
$$

$$
\hat y_{main}=\hat y_C(24\mathrm{m},\mathrm{balanced},\mathrm{architecture\ unresolved}).
$$

其中 $\mathcal P$ 是上述发布资格规则。因此表 1 的 471.8B、296 SM、768 GB 等全部主数值以及两个次级架构情景都来自路线 C；路线 A 决定这些数字不得称为“经历史回测合格的统计中心”，路线 B 决定其自身 672B 等输出只能保留为低置信诊断。A 与 B 没有进入路线 C 的目标函数、输入抽样或数值计算，也没有把 C 的区间收窄。把三路直接求均值会同时混合未定义的 A 弃权值、B 的低精度代理和 C 的稠密 FP4，并拼成没有联合可行性的伪 GPU，因而在理论与量纲上都不成立。

### 路线 A：历史环境回归如何拟合、如何失败

#### 动机

路线 A 的任务是检验一个可证伪命题：决策代理时点前可见的工作负载或供给环境变化，能否在严格时间留出中稳定预测下一代规格增速，并据此产生经历史资格审定的数值中心。它把“公开趋势看起来相关”转化为与上一代沿用、历史中位 CAGR 和平滑漂移基线的直接比较；若不能稳定胜出，模型必须弃权。

#### 假设

路线 A 假设每个目标的相邻世代年化对数增长，可在局部由少量公开环境变化率线性解释；解释变量只能在目标截点前公开，且只能用训练折中位数与稳健尺度标准化。模型不以事后答案填补空缺，也不假定六个规格共享同一斜率。截距围绕训练折的中位年化增长收缩，精度为 0.5；环境斜率的高斯 ridge 精度为 4.0；残差方差采用 shape 2.5 的 inverse-gamma 先验，年化 log-scale 下限为 0.05。一个预测变量至少需要 3 个训练转移，两个变量为保证自由度实际至少需要 4 个，即 $\max(3,p+2)$。
...
其中 $x_g$ 是环境变化率，$\alpha_k$ 是目标特定截距，$\beta_k$ 是环境斜率，$\varepsilon_{gk}$ 是残差。高斯似然与上述共轭先验产生参数后验，并把下一世代预测变换回对数规格上的 Student-$t$ 分布；每折 4096 次后验抽样用于 CRPS 与覆盖评分。

#### 理论合理性

对数差分把不同量纲规格的乘法增长转换为可比较的年化响应；训练折内标准化阻止未来信息进入尺度参数；ridge 收缩与 Student-$t$ 预测则分别缓解小样本斜率爆炸和参数不确定性。这个结构适合检验“环境变化是否增加样本外解释力”，但不能靠先验创造缺失的独立世代。因而其理论合理性必须由严格留未来评分和预声明拒绝门槛补全，而不能由样本内拟合优度代替。
...
路线 A 的预测动作是弃权：8 个变体在拟合前拒绝，10 个完成拟合变体形成的 30 个资格单元又全部失败，因此不存在可发布的 Feynman 晶体管、SM、HBM、互连或功率中心，也不存在可用于与 B/C 平均的缺失值替代项。表 1 将其首页数值资格记为 $\gamma_A^{home}=0$。

#### 分析

路线 A 能支持的结论是“现有公开历史不足以识别环境回归中心”，而不是“需求或供给与规格无关”。强共趋势、39 个预测变量空单元和极少留出折使斜率无法稳定分离。技术预测研究同样警告，少量技术世代上的指数规律可能在拟合内稳定、在样本外失效 {cite}`farmer2016predictable`。路线 A 因而改变最终结论的证据等级：路线 C 的工作点不能称为经历史回测合格的统计中心；它不改变 C 的方程、输入或数值，也不用于收窄 B 或 C。

### 路线 B：贝叶斯预测影响图如何传播、如何校验

#### 动机

路线 B 不追求从极少世代中恢复一个稳定回归斜率，而是回答另一问题：把当前可见的计算、内存、工艺、封装、互连、功率、可靠性与 TCO 压力写成显式节点后，哪些规格和硬件特性会随哪些输入上移或下移。它的价值是暴露假设传播、共同驱动和方向敏感性，并以历史留出检验这种传播是否至少与既有世代相容。

#### 假设

每个根变量被归一化到 $[0,1]$，并以“下界—众数—上界”的独立三角分布表达当前输入。五个根由公开量派生，七个根使用显式工程假设；`derived` 仍依赖归一化、等权或代理映射。模型进一步假设给定父节点后，各连续规格或二元特性按各自条件分布生成，且离散可行规格网格足以承载输出。根变量独立是计算假设，不是对真实需求、供给或客户组合相互独立的事实声明。
...
在上述根分布上，二元特性以带独立高斯系数先验的 logistic CPD 拟合，取 MAP 与 Hessian 的 Laplace 近似后抽样；连续规格对年化 log-growth 使用高斯先验线性 CPD，抽样参数与残差，再以最近对数距离投影到预声明可行规格网格。目标的六个潜在压力均值分别为 compute 0.701、memory 0.727、physical 0.665、power 0.446、risk 0.354、scale 0.702。图中没有 decision/utility 节点，根也按独立边际抽样，所以它不是已识别的 NVIDIA 决策网络。

#### 理论合理性

有向因子分解迫使每条压力—特性—规格路径显式出现，适合做“改变一个假设时下游如何响应”的可审计敏感性分析；log-growth CPD 保持连续规格的乘法语义，logistic CPD 则为二元特性提供 proper score 可评分输出。Laplace 近似和离散网格使小样本计算可执行，却不会自动校准概率：独立根可能重复放大同一公开信号，网格投影可能制造宽覆盖或边界堆积，缺少 decision/utility 节点又使它不能表示 NVIDIA 的受约束选择。因此理论上合理的用途是诊断，不是恢复内部采用概率或最终效用最优点。

#### 观测数据

目标时点的 12 个根变量、证据类别与输入范围如下；“缺失量”列明确指出每个代理没有观察到的私有或物理量。
...
路线 B 的 672B/576GB/56TB/s/200PF/7.2TB/s/2.8kW 只展示假设如何沿图传播。低精度与 HBM 容量的历史误差、两个劣于基线的特性节点、几乎全面的网格投影和 58.33% 假设根共同阻止其成为首页中心，因此 $\gamma_B^{home}=0$。这些结果可用于指出路线 C 应重点披露哪些假设和反证条件，但没有数值进入路线 C 的目标函数或输入分布；尤其 200 PF 是路线 B 的原生低精度代理，不能与路线 C 的 43.4 PFLOP/s 稠密 FP4 求平均。

### 路线 C：可行域约束设计空间如何求解

#### 动机

路线 A 已证明公开历史不足以产生合格统计中心，路线 B 又不能表示带私有效用的联合选择；但 GPU 规格仍必须同时满足晶体管、封装、HBM、互连、功率、冷却与机架边界。路线 C 因此把问题降级为可计算的工程条件句：在一个明确的历史信息截点、架构集合、目标权重和物理/系统输入范围下，哪个联合候选通过全部约束并取得最高多目标分数。

#### 假设

主条件以 2026-03-20 为信息截点。每个连续工程输入使用“下界—区间中点众数—上界”的对称三角分布，容量等少数变量按预声明离散可得性抽取；这些分布是压力测试条件，不是测量误差后验。候选架构只含 2/4 个横向 tile、1/2 个主动层及列明的 die、HBM、TDP、NVLink lane 和机架网格。五组目标权重由研究设计显式给定，不代表已识别的 NVIDIA 私有效用；成本只用 2026 美元代理，不代表售价、毛利或真实 TCO。

#### 模型

每次技术抽样先得到一个 $\theta$，再对 tile、主动层、die area、HBM、TDP、NVLink lane 与机架规模的笛卡尔网格逐个候选检查约束。裸片良率代理为 $Y_d=(1+D A_{cm^2}/\alpha)^{-\alpha}$；已知良裸片筛选使它只进入成本/可行性，不再对逻辑 die 数幂乘。晶体管容量为
...
约束优化与规格问题的物理结构一致：提高一个资源会同时消耗面积、封装边缘、功率、冷却或机架预算，所以预测对象必须是联合候选而不是各指标独立趋势。Roofline 型最小值把计算、HBM 与 NVLink 瓶颈写入交付代理；对归一化目标取加权对数可表达相对收益递减，并避免单一大尺度指标机械支配总分。其合理性仍是条件性的：输入范围、离散网格和目标权重来自公开证据与工程假设，缺少真实 BOM、热阻、良率与私有效用时，优化器只能产生可反证工作点，不能产生有历史覆盖率的统计预测。

#### 观测数据

24 个月主情景实际使用的连续输入范围与离散候选如下。每一行同时说明该输入在模型中的用途和公开证据尚未覆盖的关键量。
...
路线 C 无法对未来工程分位数做历史覆盖回测；它执行的是容量边界 sanity check。将 A100、H100、B200 与 Rubin 的已知标签代回相应历史边界，检验模型是否错误排除已存在产品。四例均通过，但这是必要而非充分条件。
...
这些检查没有历史留出误差，也没有验证封装装配、成本或热阻绝对值，因而不能把后续 q10/q50/q90 解释为校准区间。

#### 预测

路线 C 是表 1 全部主、次数值的唯一生成路径。主输出取 24m、balanced、架构未决条件的边际 q50：471.8B 晶体管、296 SM、768 GB HBM、45.3 TB/s HBM 带宽、43.4 PFLOP/s 稠密 FP4、9.18 TB/s 双向 NVLink、2.6 kW 单 GPU、72 GPU 与 323 kW 机架。固定 `2T-1L` 与 `4T-2L` 的 q50 作为较低权重架构情景；它们不是 A/B 的替代预测。
...
绑定结果说明首页工作点主要依赖封装装配、footprint 与冷却假设，而不是由单一历代 CAGR 推出。路线 C 的 q10/q50/q90 因而是“给定三角输入、架构与权重时，优化器输出的边际工程分位数”，不是置信区间、可信区间或校准概率；不同指标的边际 q50 也未必来自同一个联合候选。它向首页供应数值工作点的资格为 $\gamma_C^{home}=1$，只表示在论文预声明语义下允许发布具名工程结果，不表示路线 C 比 A/B 有 100% 概率正确。
...
定量结论首先是拒绝：路线 A 在 30 个资格单元中没有任何统计合格中心；路线 B 的影响方向可用于敏感性，但 58.33% 的目标根为显式假设且若干特性 Brier 不如基线。按照首页数值供给资格，$(A,B,C)=(0,0,1)$：A 否决“统计中心”称谓，B 保留诊断但不提供中心，最终数字只来自路线 C 的具名工程情景；这不是三模型平均。24 个月 balanced/架构未决工作点为约 471.8B 晶体管、296 SM、768 GB HBM、45.3 TB/s HBM 带宽、43.4 PFLOP/s 稠密 FP4、9.18 TB/s 双向 NVLink、2.6 kW 单 GPU、72 GPU 与 323 kW 机架；其同条件带和跨条件外包络必须与数字一起阅读，且均无覆盖概率。
