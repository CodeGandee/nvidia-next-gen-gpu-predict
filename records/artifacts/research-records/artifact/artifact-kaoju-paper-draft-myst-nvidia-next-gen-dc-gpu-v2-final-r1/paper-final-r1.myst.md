...
本文面向熟悉 GPU、LLM 系统和基本统计建模的体系结构研究者、平台架构师与容量规划者。结论不是采购建议，也不是 NVIDIA 产品事实；截至证据冻结日 2026-08-26，本地研究产物未获得外部发表或产品声明授权。
...
结果是 10 个变体完成拟合、8 个因没有可评分的滚动留出折而拒绝。10 个完成变体产生 30 个“变体×截点”资格单元，其中 6 个没有可评分折，其余单元最多只有 1–4 折；15 个单元少于最低留出折数，21 个单元的平均绝对对数误差差于最佳朴素基线，23 个单元的 CRPS 改善缺失或被单一折支配，12 个单元至少含一个预声明分布外留出点。统计合格单元数、合格变体数和获授权预测中心数均为 0。
...
当前未条件化影响图均值为：多计算裸片 0.751、HBM 代际步进 0.752、新数值格式 0.721、NVLink 代际步进 0.855、功率平滑 0.379、GPU-package CPO 0.212。GTC 2026 已明确列出 NVLink 8，只有语义完全相同的“NVLink 代际步进”节点可以固定为 true；固定后的数值 1.0 是已披露条件，不是模型置信度。由于新数值格式和未条件化 NVLink 的 Brier 均差于基线，CPO 没有正例，power smoothing 只有一个正例，正文不把上述均值发布为产品采用概率。
...
24 个月、balanced、架构未决条件下 1024/1024 样本通过模型可行性筛选。优化器的模型内部选择频率为：4 个横向 tile 83.2%、1 个主动逻辑层 91.7%、16 个 HBM stack 62.7%、72 GPU 机架 64.9%。这些比例来自输入分布和目标函数下的设计选择频率，不是 NVIDIA 采用概率。约束绑定频率中，package assembly 99.2%、package footprint 60.1%、card cooling 57.2%、rack power 31.2%、compute power 24.5%、HBM controller 15.3%、die yield 6.6%；由于缺少绝对缺陷密度、bond yield 与热阻数据，绑定频率只能解释模型内部瓶颈。
...
9.18 TB/s 是单 GPU 双向 NVLink 条件工作点；NVLink 8 代际已由官方披露，但任何带宽数字仍属模型输出。若正式单 GPU 双向带宽低于约 8.28 TB/s，主 scale-up 情景失效，需要重算并行效率、交换层与 72/144 GPU 机架选择。
...
更高功率、更大 scale-up domain 和更长训练/推理任务会提高故障隔离、遥测、可恢复执行、机密计算和资源分区的效用。A100 的 MIG 提供过平台连续性先例，但不能据此断言 Feynman 的具体 MIG 粒度或机密计算能力。本文把增强 RAS 与 rack-scale confidential computing 列为合理系统需求，不赋予官方状态或数值概率。
...
正文引用覆盖 34 项受审来源，包括 NVIDIA/TSMC/JEDEC 一手材料、LLM 与 GPU 系统论文、方法论文、数据集和标准。引用键、稳定身份、版本、公开日期、精确定位、来源状态与图表证据由同一引文映射解析；两项 partial 来源在正文邻近位置保留限制，不以完整证据列入产品事实。
...
最终试验使用答案隔离的历史面板 `real-history-panel-v2.json`，SHA-256 为 `1bed7326efaae56c85c5b4a0c9fa63bf5c2907bf1ad25126766d3dbd10e579e3`。面板的目标世代数值与特性标签均为空；目标输入只能从截点前公开变量形成。最终三路线清单 SHA-256 为 `2e107752e313d54f8b91e6662e16a717da4f0b164a19924807b1d30c760621ad`。
...
版本后缀只表示本调查中的修订序号，不表示模型路线或置信等级：Reading List r3 是 34 项最终来源总体；Claim/Evidence Ledger r2 是 18 条核心主张的审计修订；final-three-route-trial r2 是本文数字的最终试验；Field Summary、Audit Report 和本文初稿以各自 r1 起始。此前同名 r1 试验或旧图表不进入本文最终数字，后续修订必须保留旧哈希和差异。
